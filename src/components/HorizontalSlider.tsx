    ...
    >
      ...
      <div className="relative z-10 flex flex-col h-full gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            {card.appstore && (
              <a href={card.appstore} target="_blank" rel="noopener noreferrer"
                className="transition-all hover:text-[color:var(--mint)] hover:-translate-y-0.5"
                style={{ color: "var(--t2)" }}
                title="App Store">
        ...
        <div className="text-sm leading-relaxed flex-1"
          style={{ color: "var(--t4)", fontFamily: "'Inter', sans-serif" }}>
          {card.subtitle}
        </div>
        ...
        <div className="flex flex-wrap gap-3 mt-auto pt-2">
          {card.tags.map((tag) => (
            <span key={tag} className="text-xs font-medium tracking-wide transition-colors duration-300 hover:text-[color:var(--text)]"
              style={{
                color: "rgba(var(--mint-rgb),0.8)",
          ...
          {["←", "drag to explore", "→"].map((t, i) => (
            i % 2 === 0 ? (
              <motion.span key={i}
                ...
                style={{ color: "var(--mint)", fontFamily: "'Inter', sans-serif", fontSize: 13 }}>
                {t}
              </motion.span>
            ) : (
              <span key={i} className="tracking-[0.2em] uppercase"
                style={{ color: "var(--t5)", fontFamily: "'Inter', sans-serif", fontSize: 11 }}>
                {t}
              </span>
            )
          ))}
